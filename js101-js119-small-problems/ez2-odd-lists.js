...
function oddities(array) {
  ...
  for (let elem = 0; elem < array.length; elem += 2) {
    odditiesArray.push(array[elem]);
  }
  return odditiesArray;
}
...
function oddities(array) {
  ...
  for (let elem of array) {
    if ((array.indexOf(elem) % 2) === 0) odditiesArray.push(elem);
  }
  return odditiesArray;
}
...
//Using filter method
function oddities(array) {
  return array.filter((_, elem) => elem % 2 === 0);
}
